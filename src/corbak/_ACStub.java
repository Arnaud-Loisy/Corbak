 ...
 */
public class _ACStub extends org.omg.CORBA.portable.ObjectImpl implements AC {
	static final String[] _ids_list = { "IDL:corbak/AC:1.0" };
	...
	public corbak.Certificat generationCertificat(String PubKey,
		...
		while (true) {
			if (!this._is_local()) {
				...
				try {
					org.omg.CORBA.portable.OutputStream _output = this
							._request("generationCertificat", true);
				...
				org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke(
						"generationCertificat", _opsClass);
				if (_so == null)
					continue;
				corbak.ACOperations _self = (corbak.ACOperations) _so.servant;
				try {
					return _self.generationCertificat(PubKey, dateExpiration,
							ACemmetrice, sign);
				} finally {
					_servant_postinvoke(_so);
				}

 ...
 */
public class _AEStub extends org.omg.CORBA.portable.ObjectImpl implements AE {
	static final String[] _ids_list = { "IDL:corbak/AE:1.0" };
			...
			if (!this._is_local()) {
				...
				try {
					org.omg.CORBA.portable.OutputStream _output = this
							._request("authentification", true);
					...
					if (_exception_id
							.equals(corbak.authentificationEchoueeHelper.id())) {
						throw corbak.authentificationEchoueeHelper
		...
		while (true) {
			if (!this._is_local()) {
				...
				try {
					...
					String _exception_id = _exception.getId();
					if (_exception_id.equals(corbak.droitsInsufisantsHelper
							.id())) {
						throw corbak.droitsInsufisantsHelper.read(_exception
								.getInputStream());
					}
	...
	public corbak.Certificat genererCertificat(String PubKey) {
		while (true) {
			if (!this._is_local()) {
				...
				try {
					org.omg.CORBA.portable.OutputStream _output = this
							._request("genererCertificat", true);
					_output.write_string(PubKey);
					_input = this._invoke(_output);
					corbak.Certificat _arg_ret = corbak.CertificatHelper
							.read(_input);
					return _arg_ret;
				} catch (org.omg.CORBA.portable.RemarshalException _exception) {
					continue;
